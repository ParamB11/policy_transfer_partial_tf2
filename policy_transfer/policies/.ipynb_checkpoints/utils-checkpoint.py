...
def dense_params(in_size, out_size, name, weight_init=None):
    w = tf.compat.v1.get_variable(name + "/w", [in_size, out_size], initializer=weight_init)
    b = tf.compat.v1.get_variable(name + "/b", [out_size], initializer=tf.zeros_initializer())
    return w,b
