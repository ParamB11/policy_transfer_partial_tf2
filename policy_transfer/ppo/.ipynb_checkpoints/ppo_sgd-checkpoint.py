import sys #custom
# sys.path.insert(1, '/home/param/policy_transfer/policy_transfer/utils') #custom
...
import os, errno
# from utils.common import * #custom#
from policy_transfer.utils.common import * #custom
import copy
import gc
...
def learn(env, policy_func, *,
    ...
    # ----------------------------------------
    from crl_py.carl_wrapper_tf_agents_py import gymnasium_to_gym_space
    ob_space = env.observation_space
    ac_space = env.action_space

    if not isinstance(ob_space, gym.Space):
        ob_space = gymnasium_to_gym_space(ob_space)
    if not isinstance(ac_space, gym.Space):
        ac_space = gymnasium_to_gym_space(ac_space)
    ...
    clip_tf = tf.compat.v1.placeholder(dtype=tf.float32)
    ...
    for v in pi.get_trainable_variables():
        update_placeholders[v.name] = tf.compat.v1.placeholder(v.dtype, shape=v.get_shape())
