    ...
    def fit_data(self, training_X, training_Y, iter_num = 200, batch_size=64,
        ...
        for iter in range(iter_num):
            ...
            for batch in dataset.iterate_once(batch_size):
                ...
                # print('isinstance(inputs[0], np.ndarray) =', isinstance(inputs[0], np.ndarray))
...
class RegressorOptimizer:
    def __init__(self, model, updater):
        ...
        #### classifier optimization
        self.gt_state = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, model.out_dim])

        self.input_placeholders = model.inputs + [self.gt_state]
        loss = tf.compat.v1.losses.mean_squared_error(labels=self.gt_state, predictions=model.output)
